    ...
    <div className="min-h-screen bg-slate-100 py-6 print:bg-white print:py-0">
      ...
      {bySupplier.length > 1 && (
        <p className="mx-auto mb-4 max-w-[210mm] px-4 text-sm text-slate-600 print:hidden">
          This PO has {bySupplier.length} suppliers, so it prints as {bySupplier.length} pages — each supplier gets only their own lines.
        </p>
      )}
      {bySupplier.map(({ supplier, items, totals }, page) => (
        <article
          key={supplier.id}
